...
def previous_months_invoices_december(modeladmin, request, queryset):
    ...
    previous_month_patients = Patient.objects.raw("select p.id, p.name, p.first_name " +
                                                  ...
                                                  "and prest.date >= '2015-12-01'and prest.date <= '2015-12-31' " +
                                                  "and p.private_patient = 'f' " +
                                                  "and (select count(inv.id) from public.invoices_invoiceitem inv " +
                                                  "where inv.invoice_date between '2015-12-01'::DATE and '2015-12-31'::DATE " +
    ...
    for p in previous_month_patients:
        invoiceitem = InvoiceItem(patient=p,
                                  invoice_date=datetime.datetime(2015, 12, 31),
                                  invoice_sent=False,
                                  invoice_paid=False)
        invoiceitem.clean()
        invoiceitem.save()
...
def previous_months_invoices_november(modeladmin, request, queryset):
    ...
    previous_month_patients = Patient.objects.raw("select p.id, p.name, p.first_name " +
                                                  "from public.invoices_patient p, public.invoices_prestation prest " +
                                                  "where p.id = prest.patient_id " +
                                                  "and prest.date >= '2015-11-01'and prest.date <= '2015-11-30' " +
                                                  "and p.private_patient = 'f' " +
                                                  "and (select count(inv.id) from public.invoices_invoiceitem inv " +
                                                  "where inv.invoice_date between '2015-11-01'::DATE and '2015-11-30'::DATE " +
                                                  "and inv.patient_id = p.id) = 0" +
                                                  "group by p.id " +
                                                  "order by p.name")
    invoice_counters = 0
    for p in previous_month_patients:
        invoiceitem = InvoiceItem(patient=p,
                                  invoice_date=datetime.datetime(2015, 11, 30),
                                  invoice_sent=False,
                                  invoice_paid=False)
        invoiceitem.clean()
        invoiceitem.save()
