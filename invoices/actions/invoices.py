...
@transaction.atomic
def generer_forfait_aev_sept_2024(self, request, queryset):
    # only for superuser
    ...
    for patient in queryset:
        if patient.is_under_dependence_insurance:
            final_result.append(create_prestations_for_month_v2(patient, 2024, 9))
    if len(final_result) > 0:
        #  final_result string with list of invoice numbers
        invoice_numbers = ", ".join([str(x) for x in final_result])
        self.message_user(request, "Forfait AEV Sept. généré avec succès %s factures générées %s" % (
