...
def process_codes(apps, schema_editor):
    ...
    unknowns = []
    created_codes = []
    path = '2021_january_cns_codes.csv'
        ...
        for row in reader:
            if "Section" not in row[0]:
                ...
                try:
                    ...
                    new_c_code = care_code(code=code, name=row[0][0:49], description=row[0])
                    new_c_code.save()
                ...
                for v in vs:
                    if v.end_date is None and v.start_date == start_date:
                        ...
                        codes_that_are_too_old.append('%s from %s to %s' % (care_code_to_updt.code, v.start_date,
                                                                            v.end_date))
                    elif v.end_date is None and v.start_date == parse_date("2020-01-01"):
                        v.end_date = parse_date("2020-12-31")
                        v.full_clean()
                        v.save()
                        updated_codes.append('%s from %s to %s' % (care_code_to_updt.code, v.start_date, v.end_date))
                    elif v.end_date is not None \
                            and v.end_date < parse_date("2020-12-31") \
                ...
                current_validity = validity_date.objects.filter(care_code__id=care_code_to_updt.id,
                                                                start_date=parse_date("2021-1-1"),
                                                                end_date=None)
...
class Migration(migrations.Migration):
    dependencies = [
        ('invoices', '0086_rsrc_mgmt_initial_setup'),
    ]
