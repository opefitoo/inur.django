...
@transaction.atomic
def create_assurance_dependance_invoices_sept_2024(self, request, queryset):
    """
    Create AEV invoices for all patients for the month of April 2024
    """
    # get all patients
    # exit date based on timezones
    start_period = datetime(2024, 9, 1, tzinfo=timezone.utc)
    # either less or equal to end period or null
    patients = Patient.objects.filter(is_under_dependence_insurance=True).filter(
    ...
    patients = [patient for patient in patients if LongTermMonthlyActivity.objects.filter(patient=patient, year=2024, month=9).count() > 0]
    print("Remaining patients: ", len(patients))
    # create invoices for each patient
    statement = create_monthly_invoice(patients, 9, 2024)
