...
@transaction.atomic
def create_assurance_dependance_invoices_august_2024(self, request, queryset):
    """
    Create AEV invoices for all patients for the month of April 2024
    """
    # get all patients
    # exit date based on timezones
    start_period = datetime(2024, 8, 1, tzinfo=timezone.utc)
    # either less or equal to end period or null
    patients = Patient.objects.filter(is_under_dependence_insurance=True).filter(
    ...
    print("Patients found in August 2024: ", len(patients))
    # filter patients who have not had any activity in the month of August
    patients = [patient for patient in patients if LongTermMonthlyActivity.objects.filter(patient=patient, year=2024, month=8).count() > 0]
    print("Remaining patients: ", len(patients))
    # create invoices for each patient
    statement = create_monthly_invoice(patients, 8, 2024)

@transaction.atomic
def create_assurance_dependance_invoices_septembre_2023(self, request, queryset):
    """
    Create AEV invoices for all patients for the month of September 2023
    """
    # get all patients
    # exit date based on timezones
    start_period = datetime(2023, 9, 1, tzinfo=timezone.utc)
    # either less or equal to end period or null
    patients = Patient.objects.filter(is_under_dependence_insurance=True).filter(
        Q(date_of_exit__gte=start_period) | Q(date_of_exit__isnull=True)).filter(Q(date_of_death__gte=start_period) | Q(date_of_death__isnull=True))
    # create invoices for each patient
    statement = create_monthly_invoice(patients, 9, 2023)

@transaction.atomic
def create_assurance_dependance_invoices_octobre_2023(self, request, queryset):
    """
    Create AEV invoices for all patients for the month of September 2023
    """
    # get all patients
    # exit date based on timezones
    start_period = datetime(2023, 10, 1, tzinfo=timezone.utc)
    # either less or equal to end period or null
    patients = Patient.objects.filter(is_under_dependence_insurance=True).filter(
        Q(date_of_exit__gte=start_period) | Q(date_of_exit__isnull=True)).filter(Q(date_of_death__gte=start_period) | Q(date_of_death__isnull=True))
    # create invoices for each patient
    statement = create_monthly_invoice(patients, 10, 2023)

@transaction.atomic
def create_assurance_dependance_invoices_novembre_2023(self, request, queryset):
    """
    Create AEV invoices for all patients for the month of September 2023
    """
    # get all patients
    # exit date based on timezones
    start_period = datetime(2023, 11, 1, tzinfo=timezone.utc)
    # either less or equal to end period or null
    patients = Patient.objects.filter(is_under_dependence_insurance=True).filter(
        Q(date_of_exit__gte=start_period) | Q(date_of_exit__isnull=True)).filter(Q(date_of_death__gte=start_period) | Q(date_of_death__isnull=True))
...
@transaction.atomic
def create_assurance_dependance_invoices_mars_2024(self, request, queryset):
    """
    Create AEV invoices for all patients for the month of March 2024
    """
    # get all patients
    # exit date based on timezones
    start_period = datetime(2024, 3, 1, tzinfo=timezone.utc)
    # either less or equal to end period or null
    patients = Patient.objects.filter(is_under_dependence_insurance=True).filter(
        Q(date_of_exit__gte=start_period) | Q(date_of_exit__isnull=True)).filter(Q(date_of_death__gte=start_period) | Q(date_of_death__isnull=True))
    # filter patients that had no event in the month of December
    patients = [patient for patient in patients if LongTermMonthlyActivity.objects.filter(patient=patient, year=2024, month=3).count() > 0]
    # create invoices for each patient
    statement = create_monthly_invoice(patients, 3, 2024)
    return statement
...
def create_assurance_dependance_invoices_august_2023(self, request, queryset):
    """
    Create AEV invoices for all patients for the month of August 2023
    """
    # get all patients
    # exit date based on timezones
    start_period = datetime(2023, 8, 1, tzinfo=timezone.utc)
    # either less or equal to end period or null
    patients = Patient.objects.filter(is_under_dependence_insurance=True).filter(
        Q(date_of_exit__gte=start_period) | Q(date_of_exit__isnull=True)).filter(Q(date_of_death__gte=start_period) | Q(date_of_death__isnull=True))
    # create invoices for each patient
    statement = create_monthly_invoice(patients, 8, 2023)
@transaction.atomic
        ...
        if "AMD-GI" == dtl.activity.code:
            ...
            invoice_item = LongTermCareInvoiceItem.objects.create(invoice=invoice,
                                                                      ...
                                                                      long_term_care_package=LongTermPackage.objects.filter(
                                                                          code="AAII").get(),
                                                                      quantity=dtl.quantity * 2)
        elif "AMD-GDN" == dtl.activity.code:
            print(dtl.activity.code + " " + str(dtl.activity_date) + " " + str(dtl.quantity))
            # create as many invoice items as quantity
            invoice_item = LongTermCareInvoiceItem.objects.create(invoice=invoice,
                                                                      care_date=dtl.activity_date,
                                                                      long_term_care_package=LongTermPackage.objects.filter(
                                                                          code="AMDGN").get(),
                                                                      quantity=dtl.quantity * 1)
        elif "AMD-GG" == dtl.activity.code:
            print(dtl.activity.code + " " + str(dtl.activity_date) + " " + str(dtl.quantity))
            # create as many invoice items as quantity
            invoice_item = LongTermCareInvoiceItem.objects.create(invoice=invoice,
                                                                      care_date=dtl.activity_date,
